...
{
    // We'll do a naive search: try from each position in text
...
int main(int argc, char* argv[])
{
    ...
    if (outputGroup >= (int)captures.size() || !captures[outputGroup].has_value()) {
        ...
        //  However, you might do nothing or print a blank line. 
        //  It's ambiguous; let's produce no output to keep consistent with 
        //  "only produce output if a match is found"? The examples show 
        //  an example with empty output for certain group mismatch.)
        return EXIT_SUCCESS;
    }
